          ...
          .then(text => {
            ...
            return items.slice(0, 10).map(item => {
              ...
              const extractImage = (item: Element): string => {
                // Prioritize 'media:content' which is often the highest quality
                const mediaContent = item.getElementsByTagName('media:content')[0];
                if (mediaContent && mediaContent.getAttribute('url') && mediaContent.getAttribute('medium') === 'image') {
                    return mediaContent.getAttribute('url')!;
                }

                // Check for enclosure tag, a common RSS standard for attachments
                ...
                const fullContent = contentEncoded + descriptionContent;
